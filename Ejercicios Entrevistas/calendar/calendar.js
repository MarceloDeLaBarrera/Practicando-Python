...
var Calendar = (function () {
  return {
    ...
    _templates: {
      ...
      cellHighlight:
        '<div class="calendar__cell calendar__cell--highlight">{value}</div>',
      cellHead:
        '<div class="calendar__cell calendar__cell--head">{value}</div>',
      cellHour:
        '<div class="calendar__cell calendar__cell--hour">{value}</div>',
    },
    ...
    _drawHeads: function () {
      // Draw head hours
      document.getElementById("hours-left").innerHTML = this._templates.cellHead
        .split("{value}")
        .join("");
      document.getElementById("hours-right").innerHTML =
        this._templates.cellHead.split("{value}").join("");

      for (day of this._days) {
        // Draw head day
        document.getElementById("col-" + day).innerHTML =
          this._templates.cellHead.split("{value}").join(day);
      }
    ...
    _drawCells: function () {
      for (day of this._days) {
        for (hour of this._hours) {
          ...
          document.getElementById("col-" + day).innerHTML +=
            this._templates.cellDefault.split("{id}").join(id);
        }
      }
